...
> **重要说明**: 所有10个sections都支持Theme Customizer配置（通过Schema/Blocks），**没有写死的静态内容**。分类基于功能特点：
> - **Phase 1**: 基础UI组件型 - Hero轮播、促销条、横幅等界面装饰元素
> - **Phase 2**: 数据驱动型 - 从Shopify自动获取商品/集合/博客等动态数据
> - **Phase 3**: 内容展示型 - 展示用户上传的视频/评价/社交图片/帮助资源

### Phase 1: 基础可配置型 Sections（UI组件）
...
### Phase 3: 内容展示型 Sections（可配置）

> **说明**: 这些sections虽然也是通过Schema/Blocks配置，但主要展示用户自定义的内容（视频、评价、社交图片、帮助资源），而不是从Shopify获取动态商品/博客数据。所有sections都支持Theme Customizer配置，**不是写死的静态内容**。
